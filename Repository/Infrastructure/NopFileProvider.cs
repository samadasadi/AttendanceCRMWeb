...
namespace Repository.Infrastructure
{
    ...
    public class NopFileProvider : PhysicalFileProvider, INopFileProvider
    {
        ...
        public NopFileProvider(string webHostEnvironment)
            : base(webHostEnvironment)
        {
            WebRootPath = webHostEnvironment;
        }
        ...
        public virtual string Combine(params string[] paths)
        {
            ...
            var _paths = paths.SelectMany(p => IsUncPath(p) ? new[] { p } : p.Split('\\', '/')).ToArray();
            _paths[0] = _paths[0] + @"\";
            var path = Path.Combine(_paths);
            //var path = CombinePaths(_paths);
        ...
        public virtual string GetAbsolutePath(params string[] paths)
        {
            ...
            if (paths.Any() && !paths[0].ToString().Contains(WebRootPath))
