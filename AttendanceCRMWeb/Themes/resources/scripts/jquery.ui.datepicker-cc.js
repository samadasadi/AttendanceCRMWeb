...
(function ($) { // hide the namespace
    ...
    $.extend(Datepicker.prototype, {
        ...
        _localNumber: function (inst, text) {
            ...
            for (var i = 0; i <= 9; i++) str = str.replace(new RegExp(i, "g"), numbers[i]);
            return str;
        },
                    ...
                    for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
                        ...
                        for (var dow = 0; dow < 7; dow++) { // create date picker days
                            ...
                            tbody += '<td class="' +
                                ...
                                '" href="#" num="' + printDate.getDate() + '">' + this._localNumber(inst, printDate.getDate()) + '</a>')) + '</td>'; // display selectable date
            ...
            if (secondary || !changeYear)
                ...
                for (; year <= endYear; year++) {
                    html += '<option value="' + year + '"' +
                        ...
                        '>' + this._localNumber(inst, year) + '</option>';
                }
                html += '</select>';
            }
...
jQuery(function ($) {
    $.datepicker.regional['fa'] = {
        calendar: JalaliDate,
        closeText: 'بستن',
        prevText: '', //prevText: 'قبل',
        nextText: '',//prevText: 'بعد',
        currentText: 'امروز',
...
function JalaliDate(p0, p1, p2) {
    ...
    var mod = function (a, b) { return a - (b * Math.floor(a / b)); }
    ...
    this.getFullDate = function () { return this.getFullYear() + "/" + padNumber(this.getMonth() + 1) + "/" + padNumber(this.getDate()) };
    this.getFullDateGregorian = function () { return padNumber(gregorianDate.getMonth() + 1) + "/" + padNumber(gregorianDate.getDate()) + "/" + gregorianDate.getFullYear() };
...
if ($.datepicker != undefined && typeof CalendarHolidaysItems != 'undefined') {
    ...
    $.datepicker.setDefaults({
        ...
        onChangeMonthYear: function (year, month, d) {
            if ($.grep(CalendarHolidaysItems, function (x) { return x[0].indexOf(year) != -1 }).length == 0 && CalendarHolidaysItemsEmpty.indexOf(year) == -1) {
                //Selected loading Year holidays
                var e = {};
                var send_ = {};
                ...
                send_.items = e;
                $.ajax({
                    type: "POST", async: false, url: "../WebServices/Calendar_service.asmx/get_calendar_filicityByYear",
                    data: JSON.stringify(send_), contentType: "application/json; charset=utf-8", dataType: "json",
                    success: function (c) {
                        if (c.d.length == 0) {
                            CalendarHolidaysItemsEmpty.push(year);
                            // } else if (CalendarHolidaysItems.find(x => x[0].indexOf(y) != -1) == undefined) {
                        } else if ($.grep(CalendarHolidaysItems, function (x) { return x[0].indexOf(year) != -1 }).length == 0) {
                            //add to Holidays list
                            var concatArray = CalendarHolidaysItems.concat(c.d);
                            CalendarHolidaysItems = concatArray;
                        }
                    }
                });
            }
    ...
    function renderCalendarHolidaysCallback(date) {
        var regional_ = true;
        ...
        if (CalendarHolidaysItems.length > 0) {
            // var ret = CalendarHolidaysItems.find(x => x[0] === ($(this).datepicker('option', 'all').regional != "" ? jQuery.datepicker.formatDate('yy/mm/dd', date) : jQuery.datepicker.formatDate('mm/dd/yy', date)));
            var ret = $.grep(CalendarHolidaysItems, function (x) {
                return x[0] === (regional_ === false ? jQuery.datepicker.formatDate('yy/mm/dd', date) : jQuery.datepicker.formatDate('mm/dd/yy', date))
            });
            if (ret.length == 0) {
                return [true, "", ""];
            } else {
                return [true, "holidayCalendar", ret[1]];
            };
