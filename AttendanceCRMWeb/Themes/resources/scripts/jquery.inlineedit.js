...
(function ($) {
            ...
            if (event.target !== editableElement[0]) {
                switch (event.type) {
                    case 'click':
                        ...
                        maxdefault = widget.element.css("max-width");
                        widget.element.css("background-color", "#CDCDCD").css("max-width","184px");
                        break;
    ...
    $.inlineEdit.prototype = {
        ...
        save: function (elem, event) {
            ...
            hash = { value: this.encodeHtml($control.val()) };

            // save value back to control to avoid XSS
            $control.val(hash.value);
         //   $control.context.id = hash.value;
        ...
        change: function (elem, event) {
            ...
            this.timer = window.setTimeout(function () {
                ...
                elem.css("background-color", "").css("max-width",maxdefault);
        ...
        controls: {
            ...
            input: function (value) {
                ...
                if (resources.lang == 'fa') {
                    style_ = 'direction: rtl;text-align: right;';
                } else {
                    style_ = 'direction: ltr;text-align: left;';
                }
                return '<input style="' + style_ + '" type="text" value="' + value.replace(/(\u0022)+/g, '') + '">' + this.buttonHtml();
            }
        },
